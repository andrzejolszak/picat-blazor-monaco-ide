...
using System.Linq;
using Xunit;
using static PicatBlazorMonaco.Ast.DeclarationParserStringes;

namespace Tests
{
    public class DeclarationParserStringesTest
    {
        private readonly ITestOutputHelper _output;

        public DeclarationParserStringesTest(ITestOutputHelper output)
        {
            this._output = output;
        }
...
foo(P, L) :-
    ...
    call(Goal).")]
        ...
        {
            List<Declaration> res = DeclarationParserStringes.ParseDeclarations(input);
            res.Count.Should().Be(1);
            res.Single().Name.Should().Be("foo");

            List<Reference> references = DeclarationParserStringes.ParseReferences(input, res);
        ...
        public void Args()
        {
            List<Declaration> res = DeclarationParserStringes.ParseDeclarations("foo(a)?=>a. bar-->c. yar. far()=>true. dar(a, b). tar({n,m,y}, 2, 3).");
        ...
        {
            List<Declaration> res = DeclarationParserStringes.ParseDeclarations(Examples.SeveralExamples);
            res.Count.Should().Be(149);

            List<Reference> references = DeclarationParserStringes.ParseReferences(Examples.SeveralExamples, res);
...
zar ?=> foo(a, 1).
{input}";
            List<Declaration> res = DeclarationParserStringes.ParseDeclarations(program);
            res.Single(x => x.Name == "foo");
            res.Single(x => x.Name == "zar");

            List<Reference> references = DeclarationParserStringes.ParseReferences(program, res);
